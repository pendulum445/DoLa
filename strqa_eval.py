...
def clean_answer(model_pred, random_guess=False):
    ...
    if "Thus, yes." in model_pred:
        ...
        preds = model_pred.split(
            SHORT_ANSWER_TRIGGER.lower())[1].split(".")[0].strip()
    else:
        print("Warning: answer trigger not found in model prediction:",
              model_pred,
    ...
    if preds not in ["yes", "no"]:
        print("Warning: model prediction is not yes/no:",
              preds,
              "; returning no",
              flush=True)
        if random_guess:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(fp):
        ...
        with zipfile.ZipFile(
                os.path.join(args.data_path, 'strategyqa_dataset.zip'),
                'r') as zip_ref:
    ...
    for sample in tqdm(list_data_dict):
        ...
        for i in range(retry_times):
            ...
            model_completion, c_dist = llm.generate(input_text,
                                                    **generate_kwargs)
