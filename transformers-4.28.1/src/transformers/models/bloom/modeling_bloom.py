...
def build_alibi_tensor(attention_mask: torch.Tensor, num_heads: int,
...
def dropout_add(x: torch.Tensor, residual: torch.Tensor, prob: float,
...
class BloomAttention(nn.Module):
    ...
    def _split_heads(
        ...
        fused_qkv = fused_qkv.view(batch_size, seq_length, self.num_heads, 3,
                                   self.head_dim)
    ...
    def forward(
        ...
        key_layer = key_layer.permute(0, 2, 3,
                                      1).reshape(batch_size * self.num_heads,
                                                 self.head_dim, q_length)
        ...
        output_tensor = dropout_add(output_tensor, residual,
...
class BloomPreTrainedModel(PreTrainedModel):
    ...
    def _convert_to_standard_cache(
            ...
            batch_size: int) -> Tuple[Tuple[torch.Tensor, torch.Tensor]]:
        """
    ...
    def _convert_to_bloom_cache(
        ...
        # value: [batch_size, num_heads, seq_length, head_dim] -> [batch_size * num_heads, seq_length, head_dim]
        return tuple((
            layer_past[0].view(batch_size_times_num_heads, head_dim,
                               seq_length),
            layer_past[1].view(batch_size_times_num_heads, seq_length,
                               head_dim),
        ) for layer_past in past_key_value)
...
class BloomModel(BloomPreTrainedModel):
    ...
    def build_alibi_tensor(self, attention_mask: torch.Tensor, num_heads: int,
                           dtype: torch.dtype) -> torch.Tensor:
    ...
    def _prepare_attn_mask(self, attention_mask: torch.Tensor,
...
class BloomForCausalLM(BloomPreTrainedModel):
        ...
        **deprecated_arguments,
        ...
        if early_exit_layers is not None:
            ...
            for early_exit_layer in early_exit_layers:
                logits = self.lm_head(
                    transformer_outputs.hidden_states[early_exit_layer])
    ...
    def _reorder_cache(
        self, past: Tuple[Tuple[torch.Tensor, torch.Tensor],
