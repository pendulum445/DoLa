...
class DoLa:
    ...
    def generate(self,
        ...
        with torch.no_grad():
            ...
            output_str = self.tokenizer.decode(gen_sequences,
    ...
    def get_relative_top_filter(self,
        ...
        with torch.no_grad():
            ...
            if mode == 'baseline':
                ...
                for seq_i in range(prefix_ids.shape[-1] - 1,
                    ...
                    kl1 = F.kl_div(
                        log_softmax_mature_layer[None, :, :],
                        M,
                        reduction='none').mean(
                            -1)  # shape: (num_premature_layers, batch_size)
                    kl2 = F.kl_div(
                        log_softmax_premature_layers, M,
                        reduction='none').mean(
                            -1)  # shape: (num_premature_layers, batch_size)
                    js_divs = 0.5 * (
                    ...
                    js_divs = js_divs.mean(
                        -1)  # shape: (num_premature_layers,)
                ...
                for i, l in enumerate(premature_layers):
                    base_logits[i] = dict_outputs[l][0, prefix_ids.shape[-1] -
                                                     1 + i]
                final_logits = dict_outputs[mature_layer][
                    0, prefix_ids.shape[-1] - 1:-1]
                final_logits = final_logits.log_softmax(dim=-1)
